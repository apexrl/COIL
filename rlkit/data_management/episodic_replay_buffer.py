...
class EpisodicReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        self.simple_buffer = SimpleReplayBuffer(self._max_sub_buf_size, self._observation_dim, self._action_dim,
                                                self._random_seed)
        ...
        assert not use_epsilon_decay
        if use_epsilon_decay:
            self.epsilon_min = epsilon_min_or_init
            self.epsilon_decay = epsilon_decay
        else:
            self.epsilon_init = epsilon_min_or_init

        self.bc_sampling_with_rep = bc_sampling_with_rep
        self.act_dif_mode = act_dif_mode
        self.rl_traj_limit = rl_traj_limit
    ...
    def set_data(self, data, **kwargs):
        ...
        print("MIN REWARD IN DATA = ", self._data[0]["ep_rews"])
    ...
    def has_similar_traj(self, policy_func=None, trajs=None):
        ...
        try:
            ...
            raise ValueError
        return self.log_epsilon <= max_log_ratio, log_ratios, max_log_ratio

    def get_sufficient_buffer(self, epsilon=0.6, policy_func=None, **kwargs):
        """
        ...
        if self.log_epsilon is None:
            self.log_epsilon = np.log(epsilon) if self.use_epsilon_decay else self.epsilon_init

        has_traj, log_ratios, max_log_ratio = self.has_similar_traj(policy_func)

        training_mode = "rl"
        rl_traj_num = self.rl_traj_limit
        ...
        if not self.only_bc:
            if not has_traj:
                ...
                if self.use_epsilon_decay:
                    self.log_epsilon = np.maximum(self.log_epsilon - np.log(self.epsilon_decay), self.epsilon_min)
                logger.log("No such trajs! Do behavioral cloning.")
                ...
                if not self.bc_sampling_with_rep:
                ...
                selected_trajs = [self._data[ind] for ind in range(len(self._data))
                                  if self.log_epsilon <= log_ratios[ind]]
                raw_len = len(selected_trajs)
                if len(selected_trajs) <= rl_traj_num:
                    self._data = [self._data[ind] for ind in range(len(self._data))
                                  if self.log_epsilon > log_ratios[ind]]
                else:
                    selected_ind = heapq.nlargest(rl_traj_num, enumerate(log_ratios), key=lambda x: x[1])
                    selected_ind, _ = zip(*selected_ind)
                    selected_trajs = [self._data[ind] for ind in selected_ind]
                    self._data = [self._data[ind] for ind in range(len(self._data)) if ind not in selected_ind]
                min_reward = np.min([traj["ep_rews"] for traj in selected_trajs])
        else:
            # Ablation study for only bc
            raw_len = 0
            bc_traj_num = self.bc_traj_limit
            logger.log("Only bc.")
            selected_ind = heapq.nlargest(bc_traj_num, enumerate(log_ratios), key=lambda x: x[1])
        ...
        for traj in selected_trajs:
            if training_mode == "rl":
                ...
                raise NotImplementedError("Invalid training mode.")
        logger.log("Current Sufficient num: {}, Selected num: {}, Used num: {}, Remaining Num: {}, Max log ratio: {}, "
                   .format(raw_len, traj_num, self.simple_buffer.get_traj_num(), len(self._data), max_log_ratio))

        if training_mode == "rl":
            return (self.simple_buffer, selected_trajs) if return_trajs else self.simple_buffer, \
                   training_mode, traj_steps, traj_order, traj_rews, min_reward
        elif training_mode == "bc":
            return (self.bc_buffer, selected_trajs) if return_trajs else self.bc_buffer, \
                   training_mode, traj_steps, traj_order, traj_rews, min_reward
        else:
            raise NotImplementedError

    def get_simple_buffer(self):
        return self.simple_buffer
