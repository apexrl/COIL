...
class BaseAlgorithm(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if eval_sampler is None:
            ...
            if isinstance(eval_policy, list):
                eval_policy = [MakeDeterministic(epi) for epi in eval_policy]
            else:
    ...
    def start_training(self, start_epoch=0):
        ...
        for epoch in gt.timed_for(
            ...
            print((self.save_buffer_data) and (self.num_epochs % 10 == 0))
            exit(0)
            if (self.save_buffer_data) and (self.num_epochs % 10 == 0):
                assert self.save_path != None
                if not os.path.exists(self.save_path):
                    os.makedirs(self.save_path)
                self.replay_buffer.save_data(os.path.join(self.save_path, 'buffer_data_epoch_{}.pkl'.format(epoch)))
    ...
    def _special_save(self, epoch):
        snapshot_mode = logger.get_snapshot_mode()
        logger.set_snapshot_mode("all")
        ex_data = self.get_extra_data_to_save(epoch, save_all=True)
        logger.save_extra_data(ex_data, name="extra_data_%d.pkl" % epoch)
        params = self.get_epoch_snapshot(epoch, save_all=True)
        logger.save_itr_params(epoch, params)
        logger.set_snapshot_mode(snapshot_mode)

    def _try_to_eval(self, epoch, out_of_data=False):

        if self._can_evaluate():
            # save if it's time to save
            if (epoch % self.freq_saving == 0) or (epoch + 1 >= self.num_epochs) or out_of_data:
    ...
    def _start_new_rollout(self):
        if isinstance(self.exploration_policy, list):
            for pi in self.exploration_policy:
                pi.reset()
        else:
    ...
    def get_epoch_snapshot(self, epoch, save_all=False):
        ...
        if self.save_environment or save_all:
    ...
    def get_extra_data_to_save(self, epoch, save_all=False):
        """
        ...
        if self.save_environment or save_all:
            data_to_save['env'] = self.training_env
        if self.save_replay_buffer or save_all:
            data_to_save['replay_buffer'] = self.replay_buffer
        if self.save_algorithm or save_all:
